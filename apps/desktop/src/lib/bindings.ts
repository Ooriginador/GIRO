 ...
 */
async resolveWarranty(id: string, resolutionType: string, resolution: string, refundAmount: number | null, replacementCost: number | null) : Promise<Result<WarrantyClaim, AppError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("resolve_warranty", { id, resolutionType, resolution, refundAmount, replacementCost }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
