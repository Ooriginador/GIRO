 ...
 */
export type AppError = { Database: string } | { NotFound: { entity: string; id: string } } | { Duplicate: string } | { Constraint: string } | { Validation: string } | { BadRequest: string } | { RequiredField: string } | { InvalidValue: { field: string; message: string } } | { InsufficientStock: { available: number; requested: number } } | { BusinessRule: string } | "CashSessionNotOpen" | "CashSessionAlreadyOpen" | { SaleCannotBeCanceled: string } | { Unauthorized: string } | "InvalidCredentials" | "ExpiredProduct" | { DiscountExceedsLimit: { max: number } } | { StockNegative: { current: number; new: number } } | "PriceInversion" | "PermissionDenied" | { Warning: string } | { Hardware: string } | { Network: string } | "Io" | "Serialization" | { Internal: string } | { System: string } | "Sql"
/**
